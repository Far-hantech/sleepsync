...
const defaultPost = {
  title: "Optimizing Your Bedroom for Deep Sleep",
  excerpt:
    "Exploring the latest science in sleep environments, lighting, and temperature regulation.",
  image:
          ...
          <div className="absolute bottom-3 left-3 flex gap-2">
            {tags?.map((tag, index) => (
              <Badge
                key={index}
                variant="secondary"
                className="bg-background/50 backdrop-blur-sm hover:border hover:border-primary/50 hover:bg-background/80"
              >
                {tag}
              </Badge>
            ))}
